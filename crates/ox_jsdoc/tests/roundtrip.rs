...
use ox_jsdoc::decoder::helpers::{ext_offset, read_string_field, read_u32};
use ox_jsdoc::decoder::source_file::LazySourceFile;
use ox_jsdoc::format::header::SUPPORTED_VERSION_BYTE;
use ox_jsdoc::format::kind::Kind;
use ox_jsdoc::format::node_record::{
...
fn write_jsdoc_text_leaf_roundtrips() {
    ...
    assert_eq!(
        ox_jsdoc::decoder::helpers::string_payload(&sf, 1),
        Some("hello")
    );
}
...
fn next_sibling_backpatch_links_two_children() {
    ...
    use ox_jsdoc::writer::nodes::type_node::write_type_union;
...
fn lazy_decoder_reads_jsdoc_text_value() {
    use ox_jsdoc::decoder::nodes::LazyNode;
    use ox_jsdoc::decoder::nodes::comment_ast::LazyJsdocText;
...
fn lazy_decoder_tag_with_parsed_type_dispatches_correctly() {
    use ox_jsdoc::decoder::nodes::type_node::LazyTypeNode;
    ...
    let (tag_idx, _tag_ext) = ox_jsdoc::writer::nodes::comment_ast::write_jsdoc_tag(
        &mut writer,
        Span::new(0, 20),
        0,
        false,
    ...
    let _ = ox_jsdoc::writer::nodes::comment_ast::write_jsdoc_tag_name(
        &mut writer,
        Span::new(1, 6),
        tag,
        tag_name_str,
    );
    let _ = ox_jsdoc::writer::nodes::type_node::write_type_name(
        &mut writer,
        Span::new(8, 14),
        tag,
        type_str,
    ...
    use ox_jsdoc::decoder::nodes::LazyNode;
    use ox_jsdoc::decoder::nodes::comment_ast::LazyJsdocTag;
